...
public class FollowToggle
{
    ...
    public class Handler(AppDbContext context, IUserAccessor userAccessor) : IRequestHandler<Command, Result<Unit>>
    {
        ...
        {
            var oberser = await userAccessor.GetUserAsync();
            ...
            var following = await context.UserFollowings.FindAsync([oberser.Id, target.Id], cancellationToken);

            if (following == null)
            {
                context.UserFollowings.Add(new UserFollowing
                {
                    OberverId = oberser.Id,
                    TargetId = target.Id
                });
            }
            else
